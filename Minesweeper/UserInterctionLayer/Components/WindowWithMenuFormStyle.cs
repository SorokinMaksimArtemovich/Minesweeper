...
{
    public partial class WindowWithMenuFormStyle : Component
    {

        private Rectangle NewGameRectangle = new Rectangle();
        private Rectangle StatisticsRectangle = new Rectangle();
        private Rectangle ParametersRectangle = new Rectangle();
        private Rectangle ChangeUserRectangle = new Rectangle();
        private Rectangle GloryHallRectangle = new Rectangle();

        private bool NewGameHovered = false;
        private bool StatisticsHovered = false;
        private bool ParametersHovered = false;
        private bool ChangeUserHovered = false;
        private bool GloryHallHovered = false;

        private StringFormat MenuSF = new StringFormat();

        public int MenuHeight = 35;

        public bool IsClosedBottonClicked = false;

        public string ClickedButton = "DontClicked";

        public Font MenuTextFont { get; set; } = new Font("Segoe UI", 9.75F, FontStyle.Regular);
        ...
        [Description("Указывает, должна ли кнопка закрытия формы закрымать её или передовть сигнал о нажатии в переменную")]
        public bool CloseBottonShouldClose { get; set; }
        ...
        private Size IconSize = new Size(17, 17); // Размер икноки формы
        ...
        public WindowWithMenuFormStyle()
        {
            InitializeComponent();
        }
        public WindowWithMenuFormStyle(IContainer container)
        {
            DefineStyles();
        ...
        private void Form_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                // Statistics Menu Item Hovering
                if (StatisticsRectangle.Contains(e.Location))
                {
                    if (StatisticsHovered == false)
                    {
                        StatisticsHovered = true;
                ...
                {
                    if (StatisticsHovered == true)
                    {
                        StatisticsHovered = false;
                        Form.Invalidate();
                    }
                }

                // Parameters Menu Item Hovering
                if (ParametersRectangle.Contains(e.Location))
                {
                    if (ParametersHovered == false)
                    {
                        ParametersHovered = true;
                ...
                {
                    if (ParametersHovered == true)
                    {
                        ParametersHovered = false;
                        Form.Invalidate();
                    }
                }

                // Change User Menu Item Hovering
                if (ChangeUserRectangle.Contains(e.Location))
                {
                    if (ChangeUserHovered == false)
                    {
                        ChangeUserHovered = true;
                ...
                {
                    if (ChangeUserHovered == true)
                    {
                        ChangeUserHovered = false;
                        Form.Invalidate();
                    }
                }

                // Glory Hall Menu Item Hovering
                if (GloryHallRectangle.Contains(e.Location))
                {
                    if (GloryHallHovered == false)
                    {
                        GloryHallHovered = true;
                ...
                {
                    if (GloryHallHovered == true)
                    {
                        GloryHallHovered = false;
                        Form.Invalidate();
                    }
                }
            }
        ...
        private void Form_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            {
                // Close Button Click
                if (rectBtnClose.Contains(e.Location) && CloseBottonShouldClose)
                    Form.Close();
        ...
        private void Form_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Location.Y <= HeaderHeight
                ...
                && !NewGameRectangle.Contains(e.Location)
                && !StatisticsRectangle.Contains(e.Location)
                && !ParametersRectangle.Contains(e.Location)
                && !ChangeUserRectangle.Contains(e.Location)
                && !GloryHallRectangle.Contains(e.Location))
            {
                CanDragForm = true;
                clickPosition = Cursor.Position;
                moveStartPosition = Form.Location;
            ...
            if (NewGameRectangle.Contains(e.Location))
            {
                ClickedButton = "NewGame";
            }
            else if (ParametersRectangle.Contains(e.Location))
            {
                ClickedButton = "Parameters";
            }

            else if (StatisticsRectangle.Contains(e.Location))
            {
                ClickedButton = "Statistics";
            }

            else if (GloryHallRectangle.Contains(e.Location))
            {
                ClickedButton = "GloryHall";
            }

            else if (ChangeUserRectangle.Contains(e.Location))
            {
                ClickedButton = "ChangeUser";
            }
            else
            {
                ClickedButton = "DontClicked";
            }

            if (rectBtnClose.Contains(e.Location))
            {
                IsClosedBottonClicked = true;
            }
        }
        ...
        protected void DrawStyle(Graphics graph)
        {
            ...
            rectBtnClose = new Rectangle(rectHeader.Width - ControlBoxButtonsWidth, rectHeader.Y, ControlBoxButtonsWidth, 25);
            // Crosshair Structure
            ...
            rectBtnMax = new Rectangle(rectBtnClose.X - ControlBoxButtonsWidth, rectHeader.Y, ControlBoxButtonsWidth, 25);
            ...
            rectBtnMin = new Rectangle(rectBtnMax.X - ControlBoxButtonsWidth, rectHeader.Y, ControlBoxButtonsWidth, 25);
            ...
            if (Form.ControlBox == true)
            {
                ...
                graph.DrawRectangle(new Pen(btnMaximizeHovered && Form.MaximizeBox ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), rectBtnMax);
                graph.FillRectangle(new SolidBrush(btnMaximizeHovered && Form.MaximizeBox ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), rectBtnMax);
                ...
                graph.DrawRectangle(new Pen(btnMinimizeHovered && Form.MinimizeBox ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), rectBtnMin);
                graph.FillRectangle(new SolidBrush(btnMinimizeHovered && Form.MinimizeBox ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), rectBtnMin);
            ...
            MenuSF.Alignment = StringAlignment.Center;
            MenuSF.LineAlignment = StringAlignment.Center;
            int MenuItemPosition = 1;

            NewGameRectangle = new Rectangle(MenuItemPosition, HeaderHeight - MenuHeight + 1, 110, MenuHeight);
            graph.DrawRectangle(new Pen(NewGameHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), NewGameRectangle);
            graph.FillRectangle(new SolidBrush(NewGameHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), NewGameRectangle);
            graph.DrawString("новая игра", MenuTextFont, new SolidBrush(HeaderTextColor), NewGameRectangle,  MenuSF);
            MenuItemPosition += 110;

            StatisticsRectangle = new Rectangle(MenuItemPosition, HeaderHeight - MenuHeight + 1, 100, MenuHeight);
            graph.DrawRectangle(new Pen(StatisticsHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), StatisticsRectangle);
            graph.FillRectangle(new SolidBrush(StatisticsHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), StatisticsRectangle);
            graph.DrawString("стаистика", MenuTextFont, new SolidBrush(HeaderTextColor), StatisticsRectangle, MenuSF);
            MenuItemPosition += 100;

            ParametersRectangle = new Rectangle(MenuItemPosition, HeaderHeight - MenuHeight + 1, 110, MenuHeight);
            graph.DrawRectangle(new Pen(ParametersHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), ParametersRectangle);
            graph.FillRectangle(new SolidBrush(ParametersHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), ParametersRectangle);
            graph.DrawString("параметры", MenuTextFont, new SolidBrush(HeaderTextColor), ParametersRectangle, MenuSF);
            MenuItemPosition += 110;

            ChangeUserRectangle = new Rectangle(MenuItemPosition, HeaderHeight - MenuHeight + 1, 140, MenuHeight);
            graph.DrawRectangle(new Pen(ChangeUserHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), ChangeUserRectangle);
            graph.FillRectangle(new SolidBrush(ChangeUserHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), ChangeUserRectangle);
            graph.DrawString("сменить игрока", MenuTextFont, new SolidBrush(HeaderTextColor), ChangeUserRectangle, MenuSF);
            MenuItemPosition += 140;

            GloryHallRectangle = new Rectangle(MenuItemPosition, HeaderHeight - MenuHeight + 1, 110, MenuHeight);
            graph.DrawRectangle(new Pen(GloryHallHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), GloryHallRectangle);
            graph.FillRectangle(new SolidBrush(GloryHallHovered ? Color.FromArgb(100, Color.WhiteSmoke) : Color.Transparent), GloryHallRectangle);
            graph.DrawString("зал славы", MenuTextFont, new SolidBrush(HeaderTextColor), GloryHallRectangle, MenuSF);
