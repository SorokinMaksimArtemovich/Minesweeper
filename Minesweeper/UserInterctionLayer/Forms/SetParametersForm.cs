...
{
    public partial class SetParametersForm : WindowForm
    {
        ...
        {
            bool IsCorrect = true;
            int HeightCount = 0, WidthCount = 0, MinesCount = 0;
            string DificultyLevel = "";
            if (egoldsRadioButton1.IsChecked)
            {
                DificultyLevel = "Новичек";
            }
            if (egoldsRadioButton2.IsChecked)
            {
                DificultyLevel = "Любитель";
            }
            if (egoldsRadioButton3.IsChecked)
            {
                DificultyLevel = "Профессионал";
            }
            if (egoldsRadioButton4.IsChecked)
            {
                ...
                if (IsCorrect)
                {
                    DificultyLevel = "Пользовательский";
                }
            }
            if (IsCorrect)
            {
                Conector.AddUser(Conector.BufferUserName);
                Conector.BufferUserName = "";
                if (DificultyLevel == "Пользовательский")
                {
                    Conector.SetDificultyLevel(DificultyLevel, HeightCount, WidthCount, MinesCount);
                }
                else
                {
                    Conector.SetDificultyLevel(DificultyLevel);
                }
